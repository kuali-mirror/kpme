...
<c:set var="Form" value="${EdoAssignCandidateDelegateForm}"
	scope="request" />
...
<edo:edoPageLayout>
	...
	<div style="width: 100%;">
		...
		<div class="content">
			...
			<html:form action="/EdoAssignCandidateDelegate" method="post"
                ...
                <fieldset>
                    ...
                    <c:if test="${Form.candidateDelegateAdded}">
                        <span style="color: red;">Delegate has been added</span>
                    </c:if>

                    <c:if test="${Form.alreadyCandidateDelegate}">
                        ...
                        <%--
                          ...
                          <table id="delegate-table">
                              ...
                              <tbody>
                                  <c:choose>
                                      <c:when test="${fn:length(Form.name) < 1}">
                                          <tr>
                    ...
                    <table>
                        ...
                        <tr>
                                <%--   <td> <input type="image"
                                                    ...
                                                    onclick="this.form.methodToCall.value='save'; this.form.emplToAdd.value='${Form.emplId}';"
                                                    style="border: none; vertical-align: middle;" /></td>--%>
                            <td><input type="image" alt="Add Delegate for Candidate"
                                       name="Add Delegate for Candidate"
                                       src="images/buttonsmall_save.gif"
				...
				<fieldset>
					...
					<br />
					<c:if test="${Form.candidateDelegateDeleted}">
					...
					<table id="delegate-table">
						...
						<tbody>
							<c:choose>
								<c:when test="${fn:length(Form.edoCandidateDelegatesInfo) < 1}">
									<tr>
										<td colspan="4">none</td>
									</tr>
								</c:when>
								<c:otherwise>
									<c:forEach var="entry" items="${Form.edoCandidateDelegatesInfo}">
										<tr>
											<td>${entry.delegateFullName}</td>
											<td>${entry.startDate}</td>
											<td>${entry.endDate}</td>
											<td><input type="image"
												alt="Delete Delegate for Candidate"
												name="Delete Delegate for Candidate"
												src="images/buttonsmall_delete.gif"
												onclick="this.form.methodToCall.value='deleteFacultyDelegate'; this.form.emplToDelete.value='${entry.delegatePrincipalName}';"
												style="border: none; vertical-align: middle;" /></td>
										</tr>
									</c:forEach>
								</c:otherwise>
