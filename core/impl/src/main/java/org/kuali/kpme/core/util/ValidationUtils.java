   ...
   public static boolean duplicateDeptEarnCodeExists(EarnCodeSecurityBo deptEarnCode) {
	   ...
	   int count = HrServiceLocator.getEarnCodeSecurityService().getEarnCodeSecurityCount
               (deptEarnCode.getDept(), deptEarnCode.getHrSalGroup(), deptEarnCode.getEarnCode(), deptEarnCode.isEmployee() ? "1" : "0",
                       deptEarnCode.isApprover() ? "1" : "0", deptEarnCode.isPayrollProcessor() ? "1" : "0", deptEarnCode.isActive() ? "Y" : "N", deptEarnCode.getEffectiveLocalDate(), null, deptEarnCode.getGroupKeyCode());
       if(count == 1) {
    	   valid = true;
    	   count = HrServiceLocator.getEarnCodeSecurityService().getEarnCodeSecurityCount
                   (deptEarnCode.getDept(), deptEarnCode.getHrSalGroup(), deptEarnCode.getEarnCode(), deptEarnCode.isEmployee() ? "1" : "0",
                           deptEarnCode.isApprover() ? "1" : "0", deptEarnCode.isPayrollProcessor() ? "1" : "0", deptEarnCode.isActive() ? "Y" : "N", deptEarnCode.getEffectiveLocalDate(), deptEarnCode.getHrEarnCodeSecurityId(), deptEarnCode.getGroupKeyCode());
    	   if(count == 1) {
    		   valid = false;
    	   }
       } else if(count > 1) {
