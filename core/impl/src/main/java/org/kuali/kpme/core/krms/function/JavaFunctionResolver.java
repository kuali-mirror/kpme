...
public class JavaFunctionResolver extends FunctionTermResolver{
    ...
    private Object callFunction(String serviceName, String methodName,List<Object> orderedParamValues) {
        try {
            ...
            Object javaFunctionService = HrServiceLocator.getService(serviceName);
            Class javaFunctionServiceClass = javaFunctionService.getClass();
            Method method = javaFunctionServiceClass.getMethod(methodName, classtypes);
            return method.invoke(javaFunctionService,functionParamObjects.toArray());
        }catch (Exception e) {
