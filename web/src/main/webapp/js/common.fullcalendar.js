...
var defaults = {
	...
	showCustomWeekData: true,
	weekTotalArray: new Array(),
...
function Calendar(element, options, eventSources) {
	...
	//	setYMD(startDate, options.syear, options.smonth, options.sdate);
//  setYMD(endDate, options.eyear, options.emonth, options.edate);
...
//			header.activateButton(newViewName);
	...
	function renderView(inc) {
		if (elementVisible()) {
			...
			if (!currentView.start || inc || date < currentView.start || date >= currentView.end) {
				...
				currentView.render(date, startDate, endDate, inc || 0); // responsible for clearing events
				setSize(true);
	...
	function gotoDate(year, month, dateOfMonth) {
        ...
        function gotoDateRange(syear, smonth, sdateOfMonth, eyear, emonth, edateOfMonth) {
		if (syear instanceof Date) {
			date = cloneDate(syear); // provided 1 argument, a Date
		}else{
			setYMD(date, syear, smonth, sdateOfMonth);
                        setYMD(startDate, syear, smonth, sdateOfMonth);
                        setYMD(endDate, eyear, emonth, edateOfMonth);
		}
		renderView();
	}
...
function MonthView(element, calendar) {
	...
	function render(date, sdate, edate, delta) {
		if (delta) {
			addMonths(sdate, delta);
                        addMonths(sdate, delta);
		}
		var start = cloneDate(sdate, true);
                var end = cloneDate(edate, true);
		...
		addDays(visEnd, (6 - visEnd.getDay() + Math.max(firstDay, nwe)) % 7);
...
function BasicView(element, calendar, viewName) {
	...
	function updateOptions() {
		...
		if (opt('weekNumberCalculation') != 'iso') {
			weekNumberFormat = "w";
			if (opt('weekNumberCalculation') == 'counter') {
				weekNumberFormat = "c";
			} 
		} else {
			weekNumberFormat = "W";
		...
		// Set custom data with Week Number.
		showCustomWeekData = opt('showCustomWeekData');
		weekTotalArray = opt('weekTotalArray');
	}
...
//				}
                                if (!(cellDate >= t.start && cellDate <= t.end)) {
					cellClasses.push('fc-other-month');
				}
				if (+cellDate == +today) {
		...
		if (showWeekNumbers) {
			body.find('.fc-week-number > div').each(function(i, _cell) {
				...
				if(weekNumberFormat != 'c') {
					$(_cell).text(formatDate(weekStart, weekNumberFormat));
				} else {
					var weekArray = weekTotalArray.split(',');
					if(showCustomWeekData) {
						var cellText = (i+1) + "<br/>" + weekArray[i]; 
						$(_cell).html(cellText);
					} else {
						$(_cell).text(i+1);
					}
