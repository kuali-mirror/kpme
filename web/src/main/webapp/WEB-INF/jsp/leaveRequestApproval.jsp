...
<c:set var="Form" value="${LeaveRequestApprovalActionForm}"
	scope="request" />
<c:set var="KualiForm" value="${LeaveRequestApprovalActionForm}"
	scope="request" />

<tk:tkHeader tabId="leaveRequestApproval">
	<html:form action="/LeaveRequestApproval.do"
		styleId="leaveRequestApproval">
		<html:hidden property="reloadValue" value="" styleId="reloadValue" />
		<html:hidden property="hrCalendarEntryId"
			value="${Form.hrCalendarEntryId}" styleId="hrCalendarEntryId" />
		<html:hidden property="prevHrCalendarEntryId"
			value="${Form.prevHrCalendarEntryId}" styleId="prevHrCalendarEntryId" />
		<html:hidden property="nextHrCalendarEntryId"
			value="${Form.nextHrCalendarEntryId}" styleId="nextHrCalendarEntryId" />
		...
		<html:hidden property="beginDateString"
			value="${Form.beginDateString}" styleId="beginDateString" />
		<html:hidden property="endDateString" value="${Form.endDateString}"
			styleId="endDateString" />
		<html:hidden property="navigationAction" value=""
			styleId="navigationAction" />
		...
		<script
			src="${ConfigProperties.js.dir}/lm.requestApproval.backbone.js"></script>
		...
		<div class="approvals">
			<table class="navigation">
				<tbody>
					<tr>
						<td>
							<%-- pay calendar group, department and work area filters --%> <tk:approvalFilter
								calledFrom="LeaveRequestApproval" />
						</td>
						<td style="width: 1%;" />
						<td>
							<fieldset style="width: 97%;">
								<legend>Select Principal</legend>
								<table id="approvals-filter" style="width: 100%">
									<tr>
										<td>&nbsp;Principal Ids : <select id="selectedPrincipal"
						...
						<td align="right"><span> <a href="LeaveApproval.do?">Team
									Calendar</a>
						</span></td>
					</tr>
		...
		<div></div>
		<p id="validation" name="validation" class="validation"></p>

		<div>
			<tk:calendar cal="${Form.leaveRequestCalendar}" docId=""
		...
		<br />
		<div align="center">
			<input type="submit" id="actionOn_Approve" class="approve"
				value="Approve Selected" name="actionOn_Approve" />&nbsp;&nbsp; <input
				type="submit" id="actionOn_Disapprove" class="approve"
		...
		<html:textarea property="leaveRequestString"
			styleId="leaveRequestString" value="${Form.leaveRequestString}" />
	</html:form>
</tk:tkHeader>
...
<div id="cal">
	<div id="dialog-form" class="dialog-form" title="Add Ledgers:">
		<html:form action="/LeaveRequestApproval.do"
			styleId="leaveRequestApproval-form">
			...
			<div class="ui-widget timesheet-panel" id="timesheet-panel">
				<table>
					<tr>
						...
						<td><label id="leaveDate" name="leaveDate" /></td>
					</tr>
					<tr>
						<td><label for="principal-user">Principal User:</label></td>
						<td><label id="principalName" name="principalName"></label> (<label
							id="principalId" name="principalId"></label>)</td>
					</tr>
					<tr>
						<td><label for="assignment">Assignment:</label></td>
						<td><label id="assignmentTitle" name="assignmentTitle" /></td>
					</tr>
					<tr>
						<td><label for="leave">Leave:</label></td>
						<td><label id="leaveCode" name="leaveCode"></label> (<label
							id="leaveHours" name="leaveHours"></label>)</td>
					</tr>
					<tr>
						<td><label for="action">Action:</label></td>
						<td><input type="radio" name="action" id="action"
							value="Approve"> Approve <input type="radio"
							name="action" id="action" value="Disapprove"> Disapprove
							<input type="radio" name="action" id="action" value="Defer">
							Defer</td>
					</tr>
					<tr>
						<td><label for="reason">Reason:</label></td>
						<td><input type="text" name="reason" id="reason" size="50" />
						</td>
					</tr>
				</table>
			</div>
