			...
			<div>
				...
				<table>
					...
					<c:forEach var="accrualCategory" items="${Form.accrualCategories}">
						<tr>
							<td>${accrualCategory.accrualCategory}-
								${accrualCategory.descr}</td>
					...
					<c:forEach var="job" items="${Form.jobs}">
						...
						<c:if test="${fn:length(Form.jobNumberToListAssignments) > 0}">
							...
							<c:forEach var="assignment"
								items="${Form.jobNumberToListAssignments[job.jobNumber]}">
								<tr>
									<td colspan="11">
										<p>
											<strong>Work Area / Task</strong> : ${assignment.workArea} -
											${assignment.workAreaObj.description} / ${assignment.task}
										</p>
			...
			<div>
				<table class="roles">
					<tr>
						...
						<td><c:forEach var="workArea"
								items="${Form.approverWorkAreas}" varStatus="row">
                                ${workArea}
					...
					<tr>
						...
						<td><c:forEach var="workArea"
								items="${Form.reviewerWorkAreas}" varStatus="row">
                                ${workArea}
                                <c:if test="${!row.last}">,</c:if>
							</c:forEach></td>
					</tr>
					<tr>
						...
						<td><c:forEach var="dept" items="${Form.deptAdminDepts}"
								varStatus="row">
                                ${dept}
                                <c:if test="${!row.last}">,</c:if>
							</c:forEach></td>
					...
					<tr>
						...
						<td><c:forEach var="dept" items="${Form.locationAdminDepts}"
								varStatus="row">
                                ${dept}
                                <c:if test="${!row.last}">,</c:if>
							</c:forEach></td>
					...
					<tr>
						...
						<td><c:forEach var="dept" items="${Form.deptViewOnlyDepts}"
								varStatus="row">
                                ${dept}
                                <c:if test="${!row.last}">,</c:if>
							</c:forEach></td>
					...
					<tr>
						...
						<td><c:forEach var="dept"
								items="${Form.payrollProcessorDepts}" varStatus="row">
                                ${dept}
                                <c:if test="${!row.last}">,</c:if>
							</c:forEach></td>
					</tr>
