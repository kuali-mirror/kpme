...
public class LeaveSummaryServiceImplTest extends KPMETestCase {
	...
	public void testGetLeaveSummary() throws Exception {
		...
		Assert.assertTrue("There ytd dates String should be 'February 1 - March 14 2012', not " + ls.getYtdDatesString(), ls.getYtdDatesString().equals("February 1 - March 14 2012"));
		...
		Assert.assertTrue("Carry over for Row should be '0', not " + aRow.getCarryOver(), aRow.getCarryOver().equals(BigDecimal.ZERO));
		...
		Assert.assertTrue("YTD approved usage for Row should be '2', not " + aRow.getYtdApprovedUsage(), aRow.getYtdApprovedUsage().equals(new BigDecimal(2)));
		Assert.assertTrue("Leave Balance for Row should be '3', not " + aRow.getLeaveBalance(), aRow.getLeaveBalance().equals(new BigDecimal(3)));
		Assert.assertTrue("Pending Leave Accrual for Row should be '0', not " + aRow.getPendingLeaveAccrual(), aRow.getPendingLeaveAccrual().equals(BigDecimal.ZERO));
		Assert.assertTrue("Pending Leave requests for Row should be '0', not " + aRow.getPendingLeaveRequests(), aRow.getPendingLeaveRequests().equals(new BigDecimal(0)));
		...
		ls = TkServiceLocator.getLeaveSummaryService().getLeaveSummary("testUser", ce);
		Assert.assertTrue("There ytd dates String should be 'January 1 - March 14 2012', not " + ls.getYtdDatesString(), ls.getYtdDatesString().equals("January 1 - March 14 2012"));
		...
		for(LeaveSummaryRow lsRow : rows ) {
			if(lsRow.getAccrualCategory().equals("testAC")) {
				Assert.assertTrue("Carry over for Row should be '0', not " + lsRow.getCarryOver(), lsRow.getCarryOver().equals(BigDecimal.ZERO));
				...
				Assert.assertTrue("Pending Leave Accrual for Row should be '0', not " + lsRow.getPendingLeaveAccrual(), lsRow.getPendingLeaveAccrual().equals(BigDecimal.ZERO));
