...
public class LeaveDonationMaintTest extends TkTestCase{
	...
	public void testCreatingLeaveBlocks() throws Exception {
		...
		assertTrue("There are leave blocks for principal id " + "10001", leaveBlockList.isEmpty());
		leaveBlockList = TkServiceLocator.getLeaveBlockService().getLeaveBlocks("10003", START_DATE, END_DATE);
		assertTrue("There are leave blocks for principal id " + "10003", leaveBlockList.isEmpty());
		
		List<LeaveBlockHistory> historyList = TkServiceLocator.getLeaveBlockHistoryService().getLeaveBlockHistories("10001", null);
		assertTrue("There are leave block histories for princiapl id 10001", historyList.isEmpty());
		historyList = TkServiceLocator.getLeaveBlockHistoryService().getLeaveBlockHistories("10003", null);
		assertTrue("There are leave block histories for princiapl id 10003", historyList.isEmpty());
	    ...
	    setFieldValue(page, "document.newMaintainableObject.description", "test-donation");
	  	...
	  	leaveBlockList = TkServiceLocator.getLeaveBlockService().getLeaveBlocks("10001", START_DATE, END_DATE);
	  	assertTrue("There should be 1 leave blocks for emplyee '10001', not " + leaveBlockList.size(), leaveBlockList.size()== 1);
	  	LeaveBlock lb = leaveBlockList.get(0);
	  	assertTrue("Hours of the leave block for donor '10001' should be -10, not " + lb.getLeaveAmount().toString(), lb.getLeaveAmount().equals(new BigDecimal(-10)));
	  	assertTrue("Request status of the leave block for donor '10001' should be Approved, not " + lb.getRequestStatus()
	  			, lb.getRequestStatus().equals(LMConstants.REQUEST_STATUS.APPROVED));
	  	
		leaveBlockList = TkServiceLocator.getLeaveBlockService().getLeaveBlocks("10003", START_DATE, END_DATE);
		assertTrue("There should be 1 leave blocks for emplyee '10003', not " + leaveBlockList.size(), leaveBlockList.size()== 1);
		lb = leaveBlockList.get(0);
	  	assertTrue("Hours of the leave block for recipient '10003' should be 8, not " + lb.getLeaveAmount().toString(), lb.getLeaveAmount().equals(new BigDecimal(8)));
	  	assertTrue("Request status of the leave block for donor '10003' should be Approved, not " + lb.getRequestStatus()
	  			, lb.getRequestStatus().equals(LMConstants.REQUEST_STATUS.APPROVED));
	  	
		historyList = TkServiceLocator.getLeaveBlockHistoryService().getLeaveBlockHistories("10001", null);
		assertTrue("There should be 1 leave block histories for princiapl id 10001"+ historyList.size(), historyList.size()== 1);
		LeaveBlockHistory lbh = historyList.get(0);
	  	assertTrue("Hours of the leave block history for donor '10001' should be -10, not " + lbh.getLeaveAmount().toString(), lbh.getLeaveAmount().equals(new BigDecimal(-10)));
	  	assertTrue("Request status of the leave block history for donor '10001' should be Approved, not " + lbh.getRequestStatus()
	  			, lbh.getRequestStatus().equals(LMConstants.REQUEST_STATUS.APPROVED));
		historyList = TkServiceLocator.getLeaveBlockHistoryService().getLeaveBlockHistories("10003", null);
		assertTrue("There should be 1 leave block histories for princiapl id 10003"+ historyList.size(), historyList.size()== 1);
		lbh = historyList.get(0);
	  	assertTrue("Hours of the leave block history for recipient '10003' should be 8, not " + lbh.getLeaveAmount().toString(), lbh.getLeaveAmount().equals(new BigDecimal(8)));
	  	assertTrue("Request status of the leave block history for donor '10003' should be Approved, not " + lbh.getRequestStatus()
	  			, lbh.getRequestStatus().equals(LMConstants.REQUEST_STATUS.APPROVED));
	}
