...
public class AccrualServiceTest extends TkTestCase {
	...
	private void verifyLeaveBlocksForStatusChange() {
		...
		assertTrue("Requst status of leave block should be " + LMConstants.REQUEST_STATUS.APPROVED + ", not " + lb.getRequestStatus()
		...
		for(LeaveBlock aLeaveBlock : leaveBlockList) {
			...
			assertTrue("Requst status of leave block should be " + LMConstants.REQUEST_STATUS.APPROVED + ", not " + aLeaveBlock.getRequestStatus()
...
// should the accrual of the last days show up on the end day or the interval day of the last pay period?????
	...
	public void testNotEligibleForAccrualAdjustment() {
		Calendar aCal = Calendar.getInstance();
		 ...
		 leaveBlockList = TkServiceLocator.getLeaveBlockService().getLeaveBlocksForDate(PRINCIPAL_ID_7, intervalDate);
		 assertTrue("There should be 2 leave block for date " + intervalDate.toString(), leaveBlockList.size() == 2);
		 for(LeaveBlock aLeaveBlock : leaveBlockList) {
			 if(aLeaveBlock.getLeaveAmount().equals(new BigDecimal(-1))) {
				 assertTrue("Accrual category of the leave block for date  " + intervalDate.toString() + " should be '5009' , not " + lb.getAccrualCategoryId()
						 , lb.getAccrualCategoryId().equals("5009")); 
			 } else if(aLeaveBlock.getLeaveAmount().equals(new BigDecimal(32))) {
				 assertTrue("Accrual category of the leave block for date  " + intervalDate.toString() + " should be '5009' , not " + lb.getAccrualCategoryId()
						 , lb.getAccrualCategoryId().equals("5009")); 
			 } else {
				 fail("Hours of the leave block for date  " + intervalDate.toString() + " should be either 32 or -1, not " + lb.getLeaveAmount().toString());
			 }
		 }
		 ...
		 for(LeaveBlock aLeaveBlock : leaveBlockList) {
			 if(aLeaveBlock.getLeaveAmount().equals(new BigDecimal(-2))) {
				 ...
				 fail("Hours of the leave block for date  " + intervalDate.toString() + " should be either 32 or -2, not " + lb.getLeaveAmount().toString());
			 }
		 }
	}
