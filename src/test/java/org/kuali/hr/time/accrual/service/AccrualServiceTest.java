...
public class AccrualServiceTest extends TkTestCase {
	...
	public void testRunAccrualForStatusChange() {
		 ...
		 Assert.assertTrue("There should be 4 leave block history for emplyee " + PRINCIPAL_ID + ", not " + historyList.size(), historyList.size()== 4);
		 ...
		 Assert.assertTrue("There should be 12 leave block history for emplyee " + PRINCIPAL_ID + ", not " + historyList.size(), historyList.size()== 12);
	}
	private void verifyLeaveBlocksForStatusChange() {
		...
		Assert.assertTrue("There should be 4 leave blocks for emplyee 'testUser', not " + leaveBlockList.size(), leaveBlockList.size()== 4);
		...
		Assert.assertTrue("Leave Code of the leave block for date 04/01/2012 should be 'MSG', not " + lb.getEarnCode(), lb.getEarnCode().equals("MSG"));
...
// should the accrual of the last days show up on the end day or the interval day of the last pay period?????
		 ...
		 leaveBlockList = TkServiceLocator.getLeaveBlockService().getLeaveBlocksForDate(PRINCIPAL_ID_4, monthlyDate);
		 Assert.assertTrue("There should be any leave blocks for date " + monthlyDate.toString() + " for emplyee " + PRINCIPAL_ID_4, leaveBlockList.size()==0);
	}
	 ...
	 * testUser7 has two leave blocks scheduled on 04/28/2012 for 2 hours and 05/05/2012 for 5 hours, they both are NOT eligible for accrual
	 * testUser7 has one leave block scheduled on 05/22/1012 of 8 horus, it's eligible for accrual
	 * testUser7 has one leave block scheduled on 06/10/1012 of 10 horus, it's NOT eligible for accrual and we are only going to consider 8 hours time off
	...
	public void testNotEligibleForAccrualAdjustment() {
		Calendar aCal = Calendar.getInstance();
		 ...
		 Assert.assertTrue("There should be 8 leave blocks for emplyee " + PRINCIPAL_ID_7 + ", not " + leaveBlockList.size(), leaveBlockList.size()== 8);
		 ...
		 Assert.assertTrue("There should be 1 leave block for date " + intervalDate.toString(), leaveBlockList.size() == 1);
		 lb = leaveBlockList.get(0);
		 Assert.assertTrue("Hours of the leave block for date  " + intervalDate.toString() + " should be 31, not " + lb.getLeaveAmount().toString()
				 , lb.getLeaveAmount().equals(new BigDecimal(31)));
		 ...
		 Assert.assertTrue("There should be 1 leave block for date " + intervalDate.toString(), leaveBlockList.size() == 1);
		 lb = leaveBlockList.get(0);
		 Assert.assertTrue("Hours of the leave block for date  " + intervalDate.toString() + " should be 30, not " + lb.getLeaveAmount().toString()
				 , lb.getLeaveAmount().equals(new BigDecimal(30)));
	}
