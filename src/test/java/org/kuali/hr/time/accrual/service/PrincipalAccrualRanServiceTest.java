...
public class PrincipalAccrualRanServiceTest extends TkTestCase {
	...
	public void testUpdateInfo() {
		...
		assertNotNull("There should be one entry in PrincipalAccrualRan table for 'testUser'", par);
		Date aDate = new Date(par.getLastRanTs().getTime());
		SimpleDateFormat formatter = new SimpleDateFormat("MM/dd/yyyy");
		assertTrue("Date of the original entry in PrincipalAccrualRan for 'testUser' should be 05/01/2012"
		...
		assertTrue("Status should be changed for 'testUser'", rerunFlag);
		...
		assertTrue("Date of the original entry in PrincipalAccrualRan for 'testUser' should be current date"
