...
<tk:tkHeader tabId="timeDistribute" >
    ...
    <div class="timeDistribute">
			...
			<table style="border-collapse:collapse;border: 1px solid #000;">
				...
				<tbody>
					<c:forEach var="timeBlockMap" items="${Form.timeBlocksToDistribute}">
						<tr>
							<td><c:out value="${timeBlockMap.key}" /></td>
							<td><c:out value="${timeBlockMap.value.beginTimestamp}" /></td>
							<td><c:out value="${timeBlockMap.value.endTimestamp}" /></td>
							<td><c:out value="${timeBlockMap.value.hours}" /></td>
							<td>
								<input type="button" class="button" value="Edit" name="editTimeBlock" id="editTimeBlock"
								onclick="javascript:
									var urlString= extractUrlBase()+'/Clock.do?methodToCall=editTimeBlock&editTimeBlockId='+ ${timeBlockMap.value.tkTimeBlockId};
