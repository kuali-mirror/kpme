...
<tk:tkHeader tabId="leaveRequestApproval">
 ...
 <%-- Team Calendar links to the leave approval page which shows calendars of all employees --%>			
		<span class="header-title-right">
    			<a href="LeaveApproval.do?">Team Calendar</a></br>
 		</span>
		<div id="leave-req-app">		
		  <c:forEach var="employeeRow" items="${Form.employeeRows}">
			...
			<c:if test="${not empty employeeRow.leaveRequestList}">
				<html:hidden property="employeeName" value="${employeeRow.employeeName}" styleId="employeeName"/>
				<%-- name of the employee links to the employee's leave calendar tab --%>	
				<span class="header-title-left">
	      			Employee: <a href="changeTargetPerson.do?${row.UserTargetURLParams}&targetUrl=LeaveCalendar.do&returnUrl=LeaveRequestApproval.do">
	      				${employeeRow.employeeName}</a>
	   			</span>	
			   			
				<%-- display error messages generated by "take action" --%>
				<p id="validation_${employeeRow.principalId}" name="validation_${employeeRow.principalId}" class="validation_${employeeRow.principalId}"></p>
				
				<%-- the ids of the the fields are used in lm.requestApproval.js, if you need to make changes to the name/id of any fileds
				make sure you make similar changes in the javascript file
				--%>
	...
	<%-- TODO: need to review the requestId, depends on the logic defined in leave request submission --%>
		            ...
		            <tr class="noborder" >
		            	...
		            	<td></td>
		            	<td>
		            		<input type="submit" class="approve" value="Take Action" name="takeAction_${employeeRow.principalId}" id="takeAction_${employeeRow.principalId}" />
	    				</td>
		            </tr>
				</table>
			</c:if>
