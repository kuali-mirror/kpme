...
public class AssignmentRule extends MaintenanceDocumentRuleBase {
	...
	protected boolean validateWorkArea(Assignment assignment ) {
		boolean valid = ValidationUtils.validateWorkArea(assignment.getWorkArea(), assignment.getEffectiveDate());

        if (!valid) {
			this.putFieldError("workAreaId", "error.existence", "workarea '" + assignment.getWorkArea()+ "'");
		}
		return valid;
	}

	protected boolean validateTask(Assignment assignment ) {
        boolean valid = ValidationUtils.validateTask(assignment.getTask(), assignment.getEffectiveDate());

        if (!valid) {
			this.putFieldError("taskId", "error.existence", "taskId '" + assignment.getTask()+ "'");
		}
