...
//        leaveBlock.setTimestamp(TKUtils.getCurrentTimestamp());
	...
	public List<LeaveBlock> filterLeaveBlocksForTimeCalendar(List<LeaveBlock> lbs, List<String> assignmentKeys) {
    	...
    	for(LeaveBlock lb : lbs) {
    		if(lb != null) {
	    		if(lb.getLeaveBlockType().equals(LMConstants.LEAVE_BLOCK_TYPE.ACCRUAL_SERVICE)
	    			...
	    			} else if(lb.getLeaveBlockType().equals(LMConstants.LEAVE_BLOCK_TYPE.TIME_CALENDAR)) {
	    				results.add(lb);
	    			}
	    		}
    		}
